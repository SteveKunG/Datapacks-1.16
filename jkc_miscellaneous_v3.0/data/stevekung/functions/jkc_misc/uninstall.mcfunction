scoreboard objectives remove TicksPlayed
scoreboard objectives remove MinutesPlayed
scoreboard objectives remove MinutesPlayedTmp
scoreboard objectives remove DivideMinutes
scoreboard objectives remove HoursPlayed
scoreboard objectives remove HoursPlayedTmp
scoreboard objectives remove DivideHours
scoreboard objectives remove DaysPlayed
scoreboard objectives remove DaysPlayedTmp
scoreboard objectives remove DivideDays
datapack disable "file/jkc_miscellaneous_v3.0"
datapack disable "file/jkc_miscellaneous_v3.0.zip"