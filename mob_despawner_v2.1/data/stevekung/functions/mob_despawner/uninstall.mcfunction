scoreboard objectives remove despawner.ticks
scoreboard objectives remove cdespawner.ticks
scoreboard objectives remove despawner.sticks
scoreboard objectives remove despawner.gticks
scoreboard objectives remove despawner.x
scoreboard objectives remove despawner.y
scoreboard objectives remove despawner.z
scoreboard objectives remove despawner.dim
scoreboard objectives remove despawner.trigg
scoreboard players reset MobDespawnTicksTmp despawner.ticks
scoreboard players reset CMobDespawnTicksTmp cdespawner.ticks
scoreboard players reset MobDespawnTicksTmp despawner.sticks
datapack disable "file/mob_despawner_v2.1"
datapack disable "file/mob_despawner_v2.1.zip"